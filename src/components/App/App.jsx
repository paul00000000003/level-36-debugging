...
const wordGuessed = (word, guessedLetters) => {
    ...
    let guessedLetters2 = ""
    if (typeof(guessedLetters[0]) !== "undefined")
        guessedLetters2 = guessedLetters[guessedLetters.length - 1]
    let remaining = word.filter(
        letter =>
        // If the letter is guessed return false (we want to remove that then)
        !guessedLetters2.includes(letter)
    );
    // If we have letters left the word is not yet guessed
    // we kunnen natuurlijk ook gewoon de conditie erop zetten dat de twee woorden aan elkaar gelijk moeten zijn, 
    //  maar omdat we hier een voorkeur voor array methods hebben, heb ik het onderstaande er maar in gezet. 
    if (guessedLetters2.length === word.length)
...
const App = props => {
    ...
    const gameOver = gameIsOver ? ( <
        GameOver chosenWord = { game.chosenWord }
    ...
    return ( <div className = "App" >
        ...
        { gameOver } 
        <TextInput currentChosenLetter = { game.currentChosenLetter }
                   gameIsOver = { gameIsOver }
                   change = { props.chosenLetterHandler }
                   submit = { props.guessLetterHandler }
        /> 
        <ChosenWord word = { game.chosenWord }
        guessedLetters = { game.guessedLetters }/> 
        <ResetGameButton click = { props.restartGameHandler }/> 
        <GuessesLeft wrongLetters = { wrongLetters }
