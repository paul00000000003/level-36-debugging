...
class AppContainer extends React.Component {
    ...
    guessLetterHandler = event => {
        ...
        if (inputGiven) {
            ...
            if (this.state.currentChosenLetter.length > 2)
                if (newGuessedLetters.includes(this.state.currentChosenLetter))
                    alert("Dit woord zit al tussen de geraden woorden. Je kunt beter een ander woord kiezen")
                else {
                    newGuessedLetters.push(this.state.currentChosenLetter);
                }
            else alert("Het ingevoerde woord moet langer dan 2 karakters zijn")
